...
function _M.get_token(user_id)
    local token_sql = pdk.string.format("SELECT token FROM %s WHERE user_id = %s", token_table, user_id)
...
function _M.set_admin(user_id)
    local set_admin_sql = pdk.string.format("UPDATE %s SET is_owner = 1 WHERE id = %s", user_table, user_id)
...
function _M.user_delete(user_id)
    local delete_user_sql = pdk.string.format("DELETE FROM %s WHERE id = %s", user_table, user_id)
    local del_user_res, del_user_err = pdk.database.execute(delete_user_sql)
    if del_user_err then
        return 500, nil, del_user_res
    end

    local delete_roles_sql = pdk.string.format("DELETE FROM %s WHERE user_id = %s", roles_table, user_id)
...
function _M.user_info(email)
    local user_sql = pdk.string.format("SELECT * FROM %s WHERE email = '%s'", user_table, email)
